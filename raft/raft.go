...
func (r *Raft) resetRandomElectionTimeout() {
	...
	r.randomElectoinTimeout = r.electionTimeout + ra.Intn(r.electionTimeout)
}
func (r *Raft) PastElectionTimeout() bool {
	return r.electionElapsed >= r.randomElectoinTimeout
}
...
func (r *Raft) becomeCandidate() {
	...
	if r.State == StateLeader {
		panic("can't transfer leader to condidate")
	}
	r.reset(r.Term + 1)
	r.tick = r.tickElection
	r.Vote = r.id //自己给自己投票
	r.State = StateCandidate
	log.Infof("%x become candidate at term %d", r.id, r.Term)
}
...
func (r *Raft) becomeLeader() {
	...
	if r.State == StateFollower {
		panic("can't transfer follow to leader")
	}
	r.reset(r.Term)
	r.tick = r.tickElection
	r.State = StateLeader
